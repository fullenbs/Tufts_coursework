...
//  Date: 11/15/21
//
//  Purpose: Reads in a database file and then displays information based off 
//           of the given inputs. 
//
...
int main(int argc, char *argv[])
{
    ...
    Animal_Array_List all_animals; //create the array list 
    Animal_Array_List *all_animals_ptr = &all_animals;

    initialize_DB(file, all_animals_ptr);
    if (argc == 2){ //For usage case of entering a file
        all_animals_ptr->print(); //prints out all animals
    } else { //gets animals of certain classification
        do_classification_report(argv[3], all_animals_ptr);
    }   
                               ...
                               Animal_Array_List *all_animals) 
{
    ...
    Animal_Array_List *animals_in_class_ptr = &animals_in_class;
    
    int counter = 0; //relevant only if no animals match classification
    
    //Checks each animal in the database to see if it matches classification 
    //given, if it does, it's added to a new array list
    for (int i = 0; i < all_animals->num_animals(); i++){
        Animal temp = all_animals->animal_at(i);
        if (temp.classification() == classification){
            animals_in_class_ptr->add(temp);
            counter++;
        }
    }
    animals_in_class_ptr->print(); //prints out new array list
    
    if (counter == 0){ //if no animals of classification
