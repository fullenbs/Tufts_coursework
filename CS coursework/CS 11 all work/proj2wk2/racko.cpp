...
void play_the_game(RackoGameState *gs_p)
{
    unsigned int i = 0; //tracks loop
    while (i < NUMBER_OF_PLAYERS){ //This is the gameplay loop
        if (gs_p->game_deck_p->number_of_cards() != 0){ //if cards in deck
            RackoPlayer *player = gs_p->players[i]; //whose turn it is
            bool temp = player->play(); 
            if (temp == false){ //if player pressed q
                cout << "User asked to stop the game. Leaving now." << endl;
                i = NUMBER_OF_PLAYERS + 1; //breaks loop
            } else if (temp == true){ //if player chose g/d , play their turn
                if (player->has_winning_hand() == true){ //if they won
                    cout << "Congratulations " << player->player_name();
                    cout << ", you've won!!" << endl;
                    i = NUMBER_OF_PLAYERS + 1; //breaks loop
                } else{
                    i++; //goes to next player
                }
            }
            if (i == NUMBER_OF_PLAYERS){ //if at last player, reset to 0th
                i = 0;
            }
        } else { //Reached if deck empty
...
void initialize_card_decks(RackoGameState *gs_p)
{
    ...
    Card temp = gs_p->game_deck_p->draw_top_card();
    gs_p->discard_pile_p = new Deck(temp);
}
...
void initialize_players(RackoGameState *gs_p)
{
    ...
    for (unsigned int i = 0; i < NUMBER_OF_PLAYERS; i++){
        ...
        cin >> player_name;
        gs_p->players[i] = new RackoPlayer(player_name, CARDS_IN_PLAYER_HAND, 
        gs_p->game_deck_p, gs_p->discard_pile_p, false);
    }
}
