...
{
    if (mazeAt(p) == FINISH) {return true;}
    if (mazeAt(p) == REJECT) {return false;}
    if (mazeAt(p) == PATH) { return false;}
    Maze(p) = PATH;

    if (solve(p.north())) {return true;}
    if (solve(p.east())) {return true;}
    if (solve(p.south())) {return true;}
    if (solve(p.west())) {return true;}
...
int Maze::num_dots_by_rows(int row)
{
    ...
    return num_dots_in_row(row, MAX_ARRAY_SIZE) + num_dots_by_rows(row + 1);
}
...
int Maze::num_dots_in_row(int row, int col)
{
    ...
    return 0 + num_dots_in_row(row, col - 1);
}
