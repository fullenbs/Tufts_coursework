...
void Heap::downHeap(int location) {
    ...
    int loc = location;
    while (not invar){ 
        if (heap[loc] < heap[leftChild(loc)] and 
            heap[loc] < heap[rightChild(loc)]){
                invar = true;
        }
        if (max(heap[leftChild(loc)], heap[rightChild(loc)]) 
            == heap[leftChild(loc)]){
                string temp = heap[rightChild(loc)];
                heap[rightChild(loc)] = heap[loc];
                heap[loc] = temp;
                loc = rightChild(loc);
        } else {
                string temp = heap[leftChild(loc)];
                heap[leftChild(loc)] = heap[loc];
                heap[loc] = temp;
                loc = leftChild(loc);
        }
    }
