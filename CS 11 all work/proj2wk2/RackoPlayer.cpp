                           ...
                           bool graphics_on)
{
    ...
    for (unsigned int i = 0; i < num_cards; i++){
        m_hand.add_card_at(i, m_game_deck_p->draw_top_card());
    }
...
{
    its_your_turn(); //initial display
    cout << player_name() << "'s hand: ";
    m_hand.print();
    cout << endl;
    cout << "The top card on the discard pile is: ";
    cout << (m_discard_pile_p->inspect_top_card()).value() << endl;
    cout << endl;
    
    char action = discard_gamedeck_or_quit(); 
    Card temp;
    
    if (action == 'g'){ //Determines course of action
        temp = get_new_card('g');
    ...
    return true; //impossible to technically reach here unless bugged
}
...
bool RackoPlayer::has_winning_hand()
{
    int temp_i = 0;
    int temp_j = 0;
    /* Gets card at ith position, then checks if each card to right of it is
    * lower or higher. If it's lower, then no winning hand, so return false.
    * If the loop completes, then we have winning hand, so can return true */ 
    for (unsigned int i = 0; i < m_hand.num_cards(); i++){
        temp_i = (m_hand.get_card_at(i)).value();
        for (unsigned int j = i+1; j < m_hand.num_cards(); j++){
            temp_j = (m_hand.get_card_at(j)).value();
            if (temp_j < temp_i){
                return false;
            }
        }
...
char RackoPlayer::discard_gamedeck_or_quit()
{
    ...
    cout << CHOICE_PROMPT;
    cin >> choice;
    //if invalid input entered, reruns the function until proper input given
    if (choice != 'q' and choice != 'g' and choice != 'd'){
        discard_gamedeck_or_quit(); 
    } else {
        return choice; 
    }
    return choice; //failsafe, should never run unless code bugged
}
...
Card RackoPlayer::get_new_card(char choice)
{
    ...
    if (choice == 'g'){
        ...
        cout << "You drew card " << temp.value() << " from the game deck.";
        cout << endl;
        return temp;
    } else if (choice == 'd'){
        temp = m_discard_pile_p->draw_top_card();;
        cout << "You drew card " << temp.value() << " from the discard pile.";
        cout << endl;
        return temp;
    }
    return Card(-1); //Should only happen if the code is bugged
}
...
void RackoPlayer::replace_card_in_hand(Card new_card)
{
    ...
    do {     // LOOP until card chosen is one that's in the hand and discard it
        cout << REPLACE_CARD_PROMPT;
        
        //Read in number, if user enters non-int, cin goes to failstate. 
        //Clear error, read bad input user gave for the int.
        value_to_drop = read_int_from_cin();
        ...
        //Loop checks if card has same value as input and if so, swaps them
        for (unsigned int i = 0; i < m_hand.num_cards(); i++){
            if (value_to_drop == (m_hand.get_card_at(i)).value()){
                perform_swap(i, new_card);
                discarded_a_card = true;
            }
        }
...
//perform_swap (Helper for replace_card_in_hand)
//Parameters: Index where card is added and card that is being added
//Purpose: Replaces card at index with the card stored in swap
//Returns: Nothing
void RackoPlayer::perform_swap(unsigned int index, Card swap)
{
    m_hand.add_card_at(index, swap);
    Card temp = m_hand.get_card_at(index + 1);
    m_hand.remove_card_at(index + 1);
    m_discard_pile_p->put_card_on_top(temp);
}
//************************************************************************
//
//                        read_int_from_cin
